...
function EditPaymentRateForm({
  ...
  const previousPage = "../../../../";
  const paymentRateUrl = `/api/v1/payments/profile/rates/`;
  ...
  const onSubmit = async (data: ISchema) => {
    ...
    const res = await weteachApi.post("api/v1/payments/mpesa/profile/make/", {
      owner,
      teacher_profile_post,
      posting_rate: data.payment_rate,
...
export default function MakeTeacherPayment() {
  const { teacherId } = useParams();

  const [tab, setTab] = useState("create-payment-profile");
  ...
  const createPaymentProfile = async () => {
    const { data: res } = await weteachApi.post(
      "/api/v1/jobs/teacher/profile/user/",
      {
        teacher: parseInt(teacherId),
      },
    );

    sessionStorage.setItem("paymentProfileId", res.id);
    sessionStorage.setItem("userId", data.data.user.id);

    setTab("choose-payment-rate");
  };

  return (
    <FormSection title={"Post profile to public"} previousPage={"../../"}>
      <Tabs.Root value={tab}>
        <Tabs.Content value={"create-payment-profile"}>
          <button className={"btn"} onClick={() => createPaymentProfile()}>
            Proceed
          </button>
        </Tabs.Content>
        <Tabs.Content value={"choose-payment-rate"}>
          {data !== undefined ? <EditPaymentRateForm setTab={setTab} /> : null}
        </Tabs.Content>
