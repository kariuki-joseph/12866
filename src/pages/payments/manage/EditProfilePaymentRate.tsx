...
export function EditRateForm(props: { rate: PaymentRate }) {
  ...
  const onSubmit = async (data: ISchema) => {
    try {
      await weteachApi.patch(`/api/v1/payments/profile/rates/modify/${rate.id}/`, data);

      await queryClient.invalidateQueries({
        queryKey: [`api/v1/payments/profile/rates/${rate.id}`],
      });
...
export default function EditProfilePaymentRate() {
  const params = useParams();
  const url = `api/v1/payments/profile/rates/${params.rateId}`;

  const { data } = useQuery({
    queryKey: [url],
    queryFn: async () => {
      const { data } = await weteachApi.get("api/v1/payments/profile/rates/");
